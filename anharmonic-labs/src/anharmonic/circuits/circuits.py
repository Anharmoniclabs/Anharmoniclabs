"""Qiskit circuit construction for the canonical Polar."""
...
from anharmonic.polar.operator import canonical_polar_operator
...
def build_polar_circuit(size: int, *, direction: str = "forward"):
    """Build a generic Qiskit unitary circuit for the Polar.
    ...
    try:
        ...
        raise ImportError("Install the 'qiskit' extra to build Qiskit circuits") from exc

    count = qubit_count(size)
    matrix = _matrix_for_direction(size, direction)
    circuit = QuantumCircuit(count, name=f"polar_{direction}_{size}")
    circuit.append(UnitaryGate(matrix, label=f"Polar-{direction}"), range(count))
    return circuit
...
def build_qft_circuit(size: int, *, direction: str = "forward"):
    ...
    if direction not in {"forward", "inverse"}:
        raise ValueError("direction must be 'forward' or 'inverse'")
    gate = QFTGate(count)
    # Canonical Polar forward analysis uses an adjoint. Use inverse QFT for the
