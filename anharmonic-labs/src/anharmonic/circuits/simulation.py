"""Exact statevector simulation of canonical Polar matrices."""
...
def simulate_polar_statevector(
    state: ArrayLike,
    *,
    direction: str = "forward",
) -> NDArray[np.complex128]:
    """Apply the Polar through Qiskit's exact Statevector/Operator machinery."""
    try:
        from qiskit.quantum_info import Operator, Statevector
    except ImportError as exc:  # pragma: no cover - environment dependent
        raise ImportError("Install the 'qiskit' extra to run Qiskit simulation") from exc

    normalized = normalize_state(state)
    operator = canonical_polar_operator(normalized.size)
