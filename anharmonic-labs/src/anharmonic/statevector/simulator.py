...
from anharmonic.polar.canonical import canonical_polar_basis
from . import gates
...
class StatevectorSimulator:
    ...
    @classmethod
    def from_statevector(cls, state: ArrayLike, *, seed: int | None = None) -> "StatevectorSimulator":
    ...
    def reset(self, state: ArrayLike | None = None) -> "StatevectorSimulator":
        replacement = StatevectorSimulator(self.num_qubits, state, seed=self.seed)
        self._state = replacement._state
        return self

    def apply_unitary(self, matrix: ArrayLike, targets: Sequence[int]) -> "StatevectorSimulator":
    ...
    def apply(self, name: str, *qubits: int, theta: float | None = None) -> "StatevectorSimulator":
        key = name.lower()
    ...
    def polar_forward(self) -> "StatevectorSimulator":
        basis = canonical_polar_basis(self._state.size)
        return self.apply_unitary(basis.conj().T, tuple(range(self.num_qubits)))

    def polar_inverse(self) -> "StatevectorSimulator":
        basis = canonical_polar_basis(self._state.size)
