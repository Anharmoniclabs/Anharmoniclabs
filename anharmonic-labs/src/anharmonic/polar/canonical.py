"""Canonical Anharmonic Labs golden-phase polar operator.

The ``phi-grid-polar-v1`` definition deliberately has no alternative transform
or large-size fallback.
...
def _size(value: int) -> int:
    ...
    if value > MAX_DENSE_SIZE:
        raise NotImplementedError(
            f"exact canonical polar size {value} exceeds dense safety limit "
            f"{MAX_DENSE_SIZE}; no substitute transform is permitted"
        )
    return value


def phase_frequencies(size: int) -> NDArray[np.float64]:
    size = _size(size)
    return np.array([np.modf((k + 1) * PHI)[0] for k in range(size)])


def raw_phase_basis(size: int) -> NDArray[np.complex128]:
    size = _size(size)
    n = np.arange(size, dtype=np.float64)
    return np.asarray(
        np.exp(2j * np.pi * np.outer(n, phase_frequencies(size))) / np.sqrt(size),
        dtype=np.complex128,
    )
...
def _basis(size: int) -> NDArray[np.complex128]:
    phi_matrix = raw_phase_basis(size)
...
def canonical_polar_basis(size: int) -> NDArray[np.complex128]:
    """Return ``U = Phi (Phi^H Phi)^(-1/2)`` as a read-only matrix."""
    return _basis(_size(size))


def polar_forward(values: ArrayLike) -> NDArray[np.complex128]:
    vector = _vector(values, "values")
    return np.asarray(canonical_polar_basis(vector.size).conj().T @ vector)


def polar_inverse(coefficients: ArrayLike) -> NDArray[np.complex128]:
    vector = _vector(coefficients, "coefficients")
    return np.asarray(canonical_polar_basis(vector.size) @ vector)
...
def canonical_basis_sha256(size: int) -> str:
    return hashlib.sha256(
        canonical_basis_portable_bytes(canonical_polar_basis(size))
    ).hexdigest()


__all__ = [
    "PHI", "FORMULA_VERSION", "PORTABLE_HASH_DECIMALS", "canonical_polar_basis",
    "polar_forward", "polar_inverse", "canonical_basis_sha256",
    "canonical_basis_portable_bytes", "raw_phase_basis", "phase_frequencies",
]
