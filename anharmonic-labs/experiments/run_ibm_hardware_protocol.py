#!/usr/bin/env python3
"""Manual full N=2/N=4 Polar/QFT distribution and tomography protocol.

No job is submitted unless ``--submit`` is present. Credentials come only from
the environment via ``anharmonic.references.ibm_runtime.runtime_service``.
"""
...
from anharmonic.references.ibm_runtime import (
...
from anharmonic.verification.metrics import compare_distributions
...
def probes(size: int) -> dict[str, QuantumCircuit]:
    qubits = size.bit_length() - 1
    transforms = {}
    forward = build_polar_circuit(size, direction="forward")
    inverse = build_polar_circuit(size, direction="inverse")
    transforms["polar_forward"] = forward
    transforms["polar_inverse"] = inverse
    roundtrip = QuantumCircuit(qubits, name="polar_barrier_roundtrip")
    roundtrip.compose(forward, inplace=True)
    roundtrip.barrier()
    roundtrip.compose(inverse, inplace=True)
    transforms["polar_barrier_roundtrip"] = roundtrip
