...
def cases() -> list[tuple[str, np.ndarray, list[tuple[np.ndarray, tuple[int, ...]]]]]:
    ...
    for qubits in range(1, 5):
        ...
        basis = canonical_polar_basis(dimension)
        result.append((f"polar_forward_{qubits}q", state, [(basis.conj().T, tuple(range(qubits)))]))
        result.append((f"polar_inverse_{qubits}q", state, [(basis, tuple(range(qubits)))]))
        result.append((f"polar_roundtrip_{qubits}q", state, [
...
def native_evolve(state: np.ndarray, operations: list[tuple[np.ndarray, tuple[int, ...]]]) -> np.ndarray:
    simulator = StatevectorSimulator.from_statevector(state)
...
def main() -> None:
    ...
    for name, state, operations in cases():
        ...
        # Direct NumPy reference for all-qubit Polar and identity cases; Qiskit
    ...
    if importlib.util.find_spec("stim"):
        ...
        native_stim = StatevectorSimulator(4).h(0).cx(0, 3).cx(3, 1).cx(1, 2).statevector()
    ...
    output, sidecar = write_result(
        ...
        backend="anharmonic_statevector_vs_" + "_and_".join(adapters),
