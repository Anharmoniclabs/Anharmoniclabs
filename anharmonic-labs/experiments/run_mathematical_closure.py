...
from anharmonic.polar.validation import load_frozen_hashes, validate_closure

ROOT = Path(__file__).resolve().parents[1]
FIXTURE = ROOT / "tests" / "fixtures" / "polar_basis_hashes_v1.json"
SEED = 73021
...
def main() -> None:
    ...
    output, sidecar = write_result(
        ...
        backend="anharmonic_canonical_polar_numpy_eigh",
        command="python experiments/run_mathematical_closure.py",
        metrics={"sizes": records},
        passed=passed,
        conclusion="The polar definition closes numerically and matches every frozen provenance basis hash.",
