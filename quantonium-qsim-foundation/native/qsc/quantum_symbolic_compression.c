/**
 * Symbolic Compression Engine - C Implementation
 *
 * SCOPE: This is a classical O(n) algorithm that compresses a restricted
 * family of separable, product-form, phi-structured state descriptions
 ...
 * ever represented. The "qubit" parameter labels the input size only;
 * no quantum-computational speedup is claimed or implied.
 * See algorithms/rft/kernels/python_bindings/quantum_symbolic_engine.py
 * for the full scope disclaimer.
 */
...
qsc_error_t qsc_init_state(qsc_state_t* state, const qsc_params_t* params) {
    ...
    size_t requested_bytes = sizeof(qsc_complex_t) * params->compression_size;
    size_t allocation_bytes = (requested_bytes + 31u) & ~(size_t)31u;
    state->amplitudes = (qsc_complex_t*)aligned_alloc(32, allocation_bytes);
    if (!state->amplitudes) {
...
qsc_error_t qsc_compress_million_qubits(qsc_state_t* state, size_t num_qubits, size_t compression_size) {
    ...
    free(state->metadata);
    state->metadata = NULL;
    qsc_perf_stats_t* perf = (qsc_perf_stats_t*)calloc(1, sizeof(qsc_perf_stats_t));
    if (perf) {
        perf->compression_time_ms = time_ms;
        perf->total_time_ms = time_ms;
        perf->operations_per_second = time_ms > 0.0
            ? (size_t)((double)num_qubits / (time_ms / 1000.0))
            : 0;
