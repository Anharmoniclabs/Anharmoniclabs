/**
 * RFT native variant kernel interface.
 *
 * IMPORTANT DOMAIN-1 CLASSIFICATION
 * ---------------------------------
 * This C kernel exposes historical and experimental unitary/transform variants.
 * It does NOT implement the canonical square polar-normalized RFT
 *
 *     U = Phi (Phi^H Phi)^(-1/2).
 *
 * The canonical native implementation is ``src/rftmw_native/rftmw_core.hpp``.
 * Variant value 13 in this ABI is the historical unfolded phi carrier bank and
 * must never be reported as the canonical RFT.
 */
...
#define RFT_PHI 1.618033988749894848204586834366
...
#define RFT_FLAG_DEBUG              0x00000008
#define RFT_FLAG_TOPOLOGICAL        0x00000010
 ...
 * round-trip behavior does not make a variant equal to the canonical RFT.
 */
typedef enum {
    ...
    RFT_VARIANT_LEGACY_PHI_CARRIER = 13,
 ...
 * RFT_VARIANT_LEGACY_PHI_CARRIER. This alias is intentionally not exported by
 * the Python canonical API.
 */
#define RFT_VARIANT_CANONICAL RFT_VARIANT_LEGACY_PHI_CARRIER

#define RFT_DCT_NORM_FACTOR 0.7071067811865476

#define RFT_TETRA_V0_X  0.5773502691896258
...
rft_error_t rft_encode_topological_edge(rft_engine_t* engine, int qubit_id, const char* edge_id, const rft_complex_t* data, size_t data_size);
rft_error_t rft_decode_topological_edge(rft_engine_t* engine, int qubit_id, const char* edge_id, rft_complex_t* output, size_t* output_size);
rft_error_t rft_surface_code_correction(rft_engine_t* engine, int qubit_id);
rft_error_t rft_measure_topological_invariant(rft_engine_t* engine, int qubit_id, const char* invariant_type, double* result);

rft_error_t rft_init(rft_engine_t* engine, size_t size, uint32_t flags);
rft_error_t rft_cleanup(rft_engine_t* engine);
rft_error_t rft_forward(rft_engine_t* engine, const rft_complex_t* input, rft_complex_t* output, size_t size);
rft_error_t rft_inverse(rft_engine_t* engine, const rft_complex_t* input, rft_complex_t* output, size_t size);
...
rft_error_t rft_von_neumann_entropy(const rft_engine_t* engine, const rft_complex_t* state, double* entropy, size_t size);
rft_error_t rft_entanglement_measure(const rft_engine_t* engine, const rft_complex_t* state, double* entanglement, size_t size);
rft_error_t rft_validate_bell_state(const rft_engine_t* engine, const rft_complex_t* bell_state, double* measured_entanglement, double tolerance);
rft_error_t rft_validate_golden_ratio_properties(const rft_engine_t* engine, double* phi_presence, double tolerance);
