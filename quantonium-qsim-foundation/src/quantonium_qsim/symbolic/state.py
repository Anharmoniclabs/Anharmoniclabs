"""Honest representations for QSC's restricted product-state family."""
...
class PhiStructuredState:
    """Separable product state encoded by one normalized factor per label.

    ``factors[q] == (alpha_q, beta_q)``. Labels are historical QSC inputs, not
    physical-qubit claims. This class cannot represent entangled states.
    """
    ...
    @classmethod
    def from_angles(cls, theta: ArrayLike, phase: ArrayLike) -> "PhiStructuredState":
    ...
    @classmethod
    def phi_schedule(cls, num_labels: int) -> "PhiStructuredState":
        if num_labels < 1:
