"""Numerical closure and frozen-hash validation for the canonical RFT."""
...
from .canonical_operator import forward_rft, inverse_rft
...
def validate_closure(
    ...
    basis = canonical_rft_basis(size)
    identity = np.eye(size, dtype=np.complex128)
    rng = np.random.default_rng(seed)
    state = rng.normal(size=size) + 1j * rng.normal(size=size)
    state /= np.linalg.norm(state)
    transformed = canonical_rft_forward(state)
    reconstructed = canonical_rft_inverse(transformed)
    ...
    deterministic = bool(np.array_equal(basis, canonical_rft_basis(size)))
    passed = (
        np.linalg.matrix_rank(raw_phi_basis(size)) == size
    ...
    return ClosureResult(
        size=size,
        rank=int(np.linalg.matrix_rank(raw_phi_basis(size))),
...
def roundtrip_error(state: np.ndarray, operator: np.ndarray | None = None) -> float:
    ...
    transform = canonical_rft_basis(vector.size) if operator is None else operator
...
def norm_preservation_error(state: np.ndarray, operator: np.ndarray | None = None) -> float:
    vector = np.asarray(state, dtype=np.complex128)
    transform = canonical_rft_basis(vector.size) if operator is None else operator
    return float(abs(np.linalg.norm(forward_rft(vector, operator=transform)) - np.linalg.norm(vector)))
