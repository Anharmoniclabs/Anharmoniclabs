...
from quantonium_qsim.exact.gates import CX, H, X
...
def test_basis_states_and_little_endian_x() -> None:
    for index in range(16):
        ...
        assert np.array_equal(ExactSimulator(4, state).statevector(), state)
    simulator = ExactSimulator(3).x(0)
...
def test_arbitrary_unitary_and_invalid_matrix() -> None:
    simulator = ExactSimulator(2).apply_unitary(np.kron(H, X), [0, 1])
...
def test_seeded_shots_probabilities_expectation_and_density() -> None:
    simulator = ExactSimulator(2, seed=17).h(0).cx(0, 1)
