...
def test_random_circuits_against_direct_numpy(qubits: int) -> None:
    ...
    simulator = ExactSimulator(qubits, initial)
    reference = initial
...
@pytest.mark.parametrize("qubits", [1, 2, 3, 4])
def test_rft_forward_inverse_against_numpy(qubits: int) -> None:
    rng = np.random.default_rng(qubits)
    state = rng.normal(size=1 << qubits) + 1j * rng.normal(size=1 << qubits)
    state /= np.linalg.norm(state)
    basis = canonical_rft_basis(state.size)
    forward = ExactSimulator(qubits, state).rft_forward().statevector()
    assert compare_statevectors(basis.conj().T @ state, forward)["passed"]
    restored = ExactSimulator(qubits, forward).rft_inverse().statevector()
