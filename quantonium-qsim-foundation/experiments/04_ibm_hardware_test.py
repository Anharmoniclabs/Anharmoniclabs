"""Execute the canonical Quantonium RFT on IBM Quantum hardware.

This experiment performs two probes:

1. Forward RFT:
   |0...0> -> RFT forward -> measurement

2. Round trip:
   |0...0> -> RFT forward -> RFT inverse -> measurement
...
from quantonium_qsim.quantum.circuits import (
    build_rft_circuit,
    qubit_count,
)
...
def build_forward_probe(size: int) -> QuantumCircuit:
    """Prepare |0...0> and apply the canonical forward RFT."""
    circuit = QuantumCircuit(qubit_count(size), name=f"rft_forward_N{size}")
    circuit.compose(
        build_rft_circuit(size, direction="forward"),
        inplace=True,
    )
    return circuit
...
def build_roundtrip_probe(size: int) -> QuantumCircuit:
    """Apply forward and inverse RFT to |0...0>."""
    circuit = QuantumCircuit(qubit_count(size), name=f"rft_roundtrip_N{size}")
    ...
    circuit.compose(
        build_rft_circuit(size, direction="inverse"),
        inplace=True,
    )
...
def main() -> None:
    ...
    print(f"RFT dimension:    {args.size}")
    ...
    output = {
        ...
        "job_id": job_id,
        "rft_dimension": args.size,
        "logical_qubits": logical_qubits,
        "shots": args.shots,
        "optimization_level": args.optimization,
        "experiment_type": "canonical_rft_ibm_hardware",
        "implementation_note": (
    ...
    output_path = Path("results") / (
        f"04_ibm_hardware_rft_N{args.size}_{timestamp}.json"
    )
